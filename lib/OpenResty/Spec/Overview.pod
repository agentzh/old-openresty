...
And we can put them together using the C<;> as the separator to define
an action object:
...
readability. The C<PostComment> defined here takes 3 parameters,
...
any of the commands fail, the whole action would fail. Even preious
successful commands would be rolled back. That is, actions always run
in a transaction.
...
calling). Special constrains would be imposed on the length of call chain
though. There would also be some limit regarding the number of commands
grouped in an action.
...
In theory, any programming languages or tools with basic HTTP 1.0 support
would have access to all the OpenResty services.

But to make things even easier, there are currently two special OpenResty
...
Most of the sample apps' source code can be found at L<http://svn.openfoundry.org/openapi/trunk/demo/>.
