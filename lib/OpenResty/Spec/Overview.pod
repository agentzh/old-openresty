...
and the C<PostFeed> view used to generate this view has the following definition:
...
Here the C<PostFeed> view takes one optional parameter C<$count> (with the default value C<20>), which controls the number of resulting rows returned.

After creating the C<Post> feed in my C<agentzh> account, one can subscribe to the feed by the following GET request:

    GET /=/feed/Post/~/~ HTTP/1.0

Check L<http://api.eeeeworks.org/=/feed/Post/~/~> to see what the actual response looks like.
