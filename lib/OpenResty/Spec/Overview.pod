...
The OpenResty server opens a special door to JavaScript code running in a web page from the user's own domain, so as to allow REST requests get directly initiated from I<end> users' web browser.

For GET requests, it's the common practice to do cross-domain AJAX requests via dynamically-created C<< <scrip> >> tags. To help the page owner do this trick with an OpenResty server, the special C<callback> URL parameter is supported to make the server returning JSON data wrapped by C<some_callback_func(> and C<);>. For example, the request
...
where C<...> is the normal stuff in C<POST /=/...>, C<PUT /=/...>, and C<DELETE /=/...>, respectively. Some people might be nervous about GET requests doing data modification and cookies for authentication should always be excluded due to the risk of XSS attacks.

To overcome the length limit of URLs, cross-site POST AJAX interface is also supported. Basically the user could use an HTML form in his web page like below:
...
with the exception that the OpenResty serser would save the response of the former POST request and allow the user to retrieve it later (using the same C<last_response> key):

  GET /=/last/response/69bc45ec71ca7dc83cc

Note that the C<last_response> key C<69bc45ec71ca7dc83cc> used here should betruly random and unique enough.

The "last response" stuff is essential for the web page client to obtain the response for its POST request because there's no way for the JavaScript code to directly "look" into the target frame (i.e. the C<myHiddenFrame> iframe in the above sample) of another domain.

As you might have already noticed, two HTTP roundtrips to do a true POST is a little too expensive. We'll use the cross-site cookies (as well as p3p headers for IE) to transfer the response for POST to the client-side JavaScript initiator.
