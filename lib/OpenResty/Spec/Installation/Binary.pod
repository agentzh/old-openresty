...
OpenResty::Spec::Installation::Binary - Install binary distributions of OpenResty
...
根据本机的 Pg 配置情况修改配置文件 F</etc/openresty/site_openresty.conf> （修改另一个文件 F<openresty.conf> 有可能会被 F<site_openresty.conf> 覆盖的，只能作为 fall-back 使用）。具体细节请参考文档 L<OpenResty::Spec::Installation>.
...
但是测试表明，standalone 服务器在流量稍大时容易超时。对于生产应用，应使用 lighttpd + FastCGI 模式来运行。lighttpd 的具体配置请参见文档 L<OpenResty::Spec::Installation>.
