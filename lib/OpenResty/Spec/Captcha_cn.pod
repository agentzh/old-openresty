...
最终的 Captcha id 是原始的 Captcha id 明文串经过 AES 加密（通过 C<Crypt::Rijndael> 和 C<Crypt::CBC> 模块）后再 Base64 编码，最后再将 [+/=] 分别替换为 [._-] （以便在 URL 中引用）后的结果，同时在前面附加了经过相同 Base64 编码步骤的对明文串的 MD5 摘要。例如，若 Captcha id 明文串为：
...
加密的示例代码如下：
	...
	(my $result=$digest.encode_base64($cipher,""))=~y!+/=!._-!;
...
解密的示例代码如下：
	...
	my $cipher=decode_base64($cipher);
