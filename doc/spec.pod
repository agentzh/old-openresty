...
与 C<GET> 相近，但对于返回列表型数据的读取操作而言，
仅返回第一个列表项。类似 SQL 语言中的 C<select top 1>.
...
该选项仅作用于 GET 请求返回列表的情形。它用于指定返回的条目数，缺省为 20。最大值为 1000。

示例：

   GET /=/model/Postbook/body/People?count=20

count 的取值须为小于上限 1000 的正整数，否则服务器将报错。

=item order_by

指定排序项（可指定多个，以逗号分隔). 例如：

    GET /=/model/Bookmark/title/Yahoo?op=like&order_by=title:asc,url:desc
