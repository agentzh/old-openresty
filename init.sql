drop schema if exists _register cascade;
...
create or replace function public.xuncomment(username text, module text, col text, out ret boolean)
returns boolean as $$
declare 
  id integer;
  sid integer;
...
begin
  ...
  sql := 'delete from pg_description where objoid = '||id||' and objsubid = '||sid;
  raise notice 'sql %', sql;
  execute sql;
  ret := true;
  return; 
