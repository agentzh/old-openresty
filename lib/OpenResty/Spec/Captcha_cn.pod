...
Captcha id 明文格式为：
	...
	<lang> <\001> <solution> <\001> <min_ts> <\001> <max_ts> <\001> <rand>
...
最终的 Captcha id 是原始的 Captcha id 明文串经过 AES 加密（通过 C<Crypt::Rijndael> 和 C<Crypt::CBC> 模块）后再 Base64 编码，最后再将 [+/=] 分别替换为 [._-] （以便在 URL 中引用）后的结果。例如，若 Captcha id 明文串为：

	cn<\001>测试一下<\001>1208357712<\001>1208361326<\001>15768

若选择密钥为 16 个 a（128 bits 密钥），则最终生成的 Captcha id 将为：

	gi2g3CgKpznmIjUq_OVsu.NzlRlFAWGWxjWLCrMmZXkNocpoe2JqN7Zg7BOvVBPQ

加密的示例代码如下：

	use Crypt::CBC;
	use MIME::Base64;
	my $plain=join("\001",'cn','测试一下','1208357712','1208361326','15768');
	my $key='a' x 16;
	...
	(my $result=encode_base64($cipher))=~y!+/=!._-!;
...
解密的示例代码如下：
	...
	use Crypt::CBC;
	use MIME::Base64;
	my $result='gi2g3CgKpznmIjUq_OVsu.NzlRlFAWGWxjWLCrMmZXkNocpoe2JqN7Zg7BOvVBPQ';
	my $key='a' x 16;
	my $algo=Crypt::CBC->new(
			-key=>$key,
			-header=>'none',
	...
	my $cipher=decode_base64($base64);
	my ($lang,$solution,$min_ts,$max_ts,$rand)=split("\001",$algo->decrypt($cipher));
	print "$lang,$solution,$min_ts,$max_ts,$rand\n";
