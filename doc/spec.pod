...
默认情况下，URL 及请求数据中的非 ASCII 字符都按 UTF-8 编码处理。如若需要使用其他编码，如 GBK, Big5, 和 Latin1 的话，需要显式地通过 C<charset> 参数指定，例如：
    ...
    POST /=/model/Bookmark.json?charset=GBK
...
可以通过下面的接口修改已有模型的某一列的名字，类型，或标签：
    ...
    POST /=/model/Bookmark/title.json
...
一个 JSON 格式的例子为：
    ...
    [
        { description: "My favorite bookmark", name: "Bookmark", url: "/=/model/Bookmark.json", writable: 1, readable: 1 },
        { description: "My favorite music", type: "model", name: "Music", url: "/=/model/Music.json", writable: 1, readable: 1 },
        { description: "My frequently accessed blog", name: "Blog", url: "/=/model/Blog.json", writable: 1, readable: 1 },
    ]
...
一个 JSON 的例子如下：
...

    {
      ...
      columns:
        [
          { name: "id", type: "serial", url: "/=/model/Bookmark/id.json", writable: 1, readable: 1 },
          { name: "title", type: "string", url: "/=/model/Bookmark/string.json", writable: 1, readable: 1 },
          { name: "url", type: "string", url: "/=/model/Bookmark/url.json", writable: 1, readable: 1 },
          { name: "description", type: "string", url: "/=/model/Bookmark/description.json", writable: 1, readable: 1 },
        ]
    }
