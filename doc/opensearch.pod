...
OpenSearch System Specification [draft] - OpenSearch  系统白皮书（草案）
...
本文定义了 OpenSearch 的系统结构, 协议层由 OpenAPI 支撑, 存储上使用的是PostgreSQL集群.
...
能够支持 OpenAPI 中对数据的所有操作, 在保证 OpenAPI 的足够灵活的基础上，同时通过PostgreSQL
的存储过程，或者视图来简化 OpenAPI 对于OpenSearch的复杂操作逻辑.
