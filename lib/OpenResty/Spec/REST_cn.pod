...
如果无法确定编码应该是什么，可以使用 C<charset=guessing> 或者 C<charset=guess>，服务器端会根据传入编码进行判断。
...
使用 C<op> 参数的时候，只能有一个查询条件，但可以添加约束信息（如 C<order_by> 等）。并且每次查询请求最多只能有一个 C<op> 的查询限制。
...
值得提醒的是，虽然，C<op=contains> 非常方便，但存在效率上的问题，比如很难高效地使用 DB 索引。因此，对于真实的应用，应该尽量考虑使用 tsearch2 这样的全文索引模块。OpenResty 提供了对 tsearch2 的全面支持.
...
通过指定 C<extended=1> 选项，可以启用扩展查询语法。下面是几个例子：
...
示例：
    ...
    { "description": "Action for removing bookmarks",
      "parameters":[ {"name":"pattern","type":"literal"} ],
      "definition":
...
指定排序项（可指定多个，以逗号分隔). 例如：
    ...
    GET /=/model/Bookmark/title/Yahoo?order_by=title:asc,url:desc
