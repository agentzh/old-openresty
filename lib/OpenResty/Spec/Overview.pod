...
In the above example, the C<user> parameter has the value C<agentzh.Public> where C<agentzh> is the account name and C<Public> the role name. In addition, the C<Public> role is an anonymous role, or a C<password> or a C<captcha> parameter would be required here as well. This authentication method is called "per-request login".
...
For convenience, the sample HTTP requests given throughout this document will not specify the C<user> nor the C<session> parameter explicitly.
...
This is approximately the C<Post> model used in my personal blog site L<http://blog.agentzh.org>. The rough SQL equivalence could be as follows:
...
The model API not only offers interfaces to perform CRUD operations on models, columns, and rows, but also gives some simple but still powerful query functionalities. Here's an example:
    ...
    GET /=/model/Post/author/agentzh?order_by=created:desc&offset=40&count=10 HTTP/1.0
